...
const PropertyTaxCalculator = () => {
  ...
  const oneHouseTaxRates = [
    ...
    { min: 60000000, max: 150000000, rate: 0.1, deduction: 30000 },
    { min: 150000000, max: 300000000, rate: 0.2, deduction: 12000000 },
    { min: 300000000, max: Infinity, rate: 0.35, deduction: 42000000 }
  ];

  // 과세표준에 따른 세율 계산
  const calculateTaxRate = (taxBase, ratesTable = taxRates) => {
  ...
  const calculateOneHouseTax = (marketValue, realRate, fairRate) => {
    ...
    // 1주택자 특례세율 적용
    const { rate, deduction } = calculateTaxRate(taxBaseCapped, oneHouseTaxRates);
    const tax = (taxBaseCapped * rate) - deduction;
  ...
  // 1주택자 계산
  // 현재 설정으로 계산 (변경된 현실화율과 공정시장가액비율 적용)
  const currentOneHouseCalc = calculateOneHouseTax(marketValue, realityRate, oneHouseFairMarketRate);

  // 기존 기준으로 계산 (비교용 - 현재 기준 그대로)
  const baseOneHouseCalc = calculateOneHouseTax(marketValue, CURRENT_REALITY_RATE, CURRENT_ONE_HOUSE_FAIR_MARKET_RATE);
  ...
  return (
    <div className="calculator-container">
      ...
      {/* 계산 결과 - 1주택자 */}
      {assessedPrice && (
        <div className="results-section results-section-one-house">
          <h3>1세대 1주택 (특례세율 적용)</h3>
          ...
          <div className="horizontal-scroll-wrapper">
            <div className="result-comparison">
              <div className="result-card base-result">
                ...
                <div className="result-item">
                  <span className="label">역산 시가:</span>
                  <span className="value">{formatNumber(baseOneHouseCalc.marketValue)}원</span>
                </div>
                <div className="result-item">
                  <span className="label">주택가격 (현 기준):</span>
              ...
              <div className="result-card current-result">
                ...
                <div className="result-item">
                  <span className="label">역산 시가:</span>
                  <span className="value">{formatNumber(currentOneHouseCalc.marketValue)}원</span>
                </div>
                <div className="result-item">
                  <span className="label">주택가격 (변경 기준):</span>
